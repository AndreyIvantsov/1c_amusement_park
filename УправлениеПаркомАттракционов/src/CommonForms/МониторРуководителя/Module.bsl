...
Процедура ПосещениеАттракционовВыбор(Элемент, ЗначениеДиаграммы, СтандартнаяОбработка)
	...
	ОткрытьФорму("Отчет.ДеталиПоПосещению.ФормаОбъекта", ПараметрыФормы);
...
Процедура СформироватьДиаграммуРаспределениеАттракционов()
	...
	Пока ВыборкаВидыАттракционов.Следующий() Цикл
		...
		Выборка = ВыборкаВидыАттракционов.Выбрать();
		Пока Выборка.Следующий() Цикл
			...
			РаспределениеАттракционов.УстановитьЗначение(Точка, Серия, Выборка.КоличествоПосещений);
		КонецЦикла;
...
Функция НастройкиОтчетаДетализацияПосещений(Аттракцион, День)
	...
	Отчет = Отчеты.ДеталиПоПосещению.Создать();
	Настройки = Отчет.КомпоновщикНастроек.Настройки;
	ПользовательскиеНастройки = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки;

    // Установка параметра Период
	ПарметрПериод = Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Период"));
	ПользовательскийПараметрПериод = ПользовательскиеНастройки.Элементы.Найти(
		ПарметрПериод.ИдентификаторПользовательскойНастройки);
	...
	Для Каждого ЭлементОтбора Из Настройки.Отбор.Элементы Цикл
		Если ЭлементОтбора.ЛевоеЗначение = ПолеДляОтбора Тогда
			ПользовательскийПараметрАттракцион = ПользовательскиеНастройки.Элементы.Найти(
